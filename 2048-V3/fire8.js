...
// ---------------- SAVE HIGHEST TILE ----------------
async function saveHighestTileToCloud(level, tile) {
  if (!window.currentUser) return;
  ...
  try {
    await runTransaction(db, async (tx) => {
      const snap = await tx.get(scoreRef);
      const currentBestTile = snap.exists() ? snap.data().highestTile || 0 : 0;
      ...
      if (tile > currentBestTile) {
        ...
        // Update localStorage for quick UI access
        localStorage.setItem(`cloud-highestTile-${window.currentUser.uid}-${level}`, tile);
        console.log("🔥 Highest tile saved:", level, tile);
      }
    });
  } catch (err) {
    console.error("❌ Failed to save highest tile:", err);
  }
}
window.saveHighestTileToCloud = saveHighestTileToCloud;

// ---------------- SAVE Longest Session ----------------
async function saveLongestTimeToCloud(level, time) {
  if (!window.currentUser) return;
  ...
  try {
    await runTransaction(db, async (tx) => {
      const snap = await tx.get(scoreRef);
      const currentLongest = snap.exists() ? snap.data().longestTime || 0 : 0;

      // Only update if new time is greater (longer)
      if (time > currentLongest) {
        ...
        localStorage.setItem(`cloud-longestTime-${window.currentUser.uid}-${level}`, time);
        console.log("🔥 Longest time saved:", level, time);
      }
    });
  } catch (err) {
    console.error("❌ Failed to save longest time:", err);
  }
}
window.saveLongestTimeToCloud = saveLongestTimeToCloud;


//======= Fastest Time to Reach 2048 ==========
async function saveFastestTimeToCloud(level, time) {
  if (!window.currentUser) return;
  ...
  try {
    await runTransaction(db, async (tx) => {
      const snap = await tx.get(scoreRef);
      const currentFastest = snap.exists() ? snap.data().fastest2048Time || Infinity : Infinity;

      // Only update if new time is lower (faster)
      if (time < currentFastest) {
        tx.set(scoreRef, {
          uid: window.currentUser.uid,
          username: window.currentUsername || null,
          level,
          fastest2048Time: time,
          updated: serverTimestamp()
        }, { merge: true });

        // Update localStorage for quick UI access
        localStorage.setItem(`cloud-fastest2048Time-${window.currentUser.uid}-${level}`, time);
        console.log("🔥 Fastest 2048 time saved:", level, time);
      }
    });
  } catch (err) {
    console.error("❌ Failed to save fastest 2048 time:", err);
  }
}
window.saveFastestTimeToCloud = saveFastestTimeToCloud;
