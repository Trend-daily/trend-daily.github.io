document.addEventListener("DOMContentLoaded", () => {
  ...
  // =============================================
  // 2048 EVOLVED — FINAL GOD-TIER VERSION By Clinton Nwezeaku
  // =============================================
  ...
  function render() {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      if (val) {
        ...
        if (prevVal === 0 && val !== 0) tile.classList.add('new');
        else if (val === prevVal * 2 || (val === 2 && prevVal === 1)) tile.classList.add('merge');
      }
  ...
  // GRIDER EXPLOSION
  function triggerGriderEffect(tileElement) {
    const effect = document.createElement('div');
    effect.className = 'grider-effect';
    tileElement.style.position = 'relative';
    tileElement.appendChild(effect);
  ...
  grid.addEventListener('click', (e) => {
    ...
    if (activeGem === 'switcher') {
      ...
      if (matrix[r][c] !== 0) {
        matrix[r][c] = 0;
        triggerGriderEffect(e.target);     // RED IMPLOSION
        gems.grider--;
        deactivateGem();
        playSound('destroy');
      }
      ...
      triggerBombShockwave();                // PINK SHOCKWAVE
      gems.bomb--;
      deactivateGem();
      playSound('bomb');
    }
  ...
  function trySpawnGem() {
    if (Math.random() < 0.18) {
      ...
      let type;
      if (rand < 40) type = 'switcher';
      else if (rand < 79) type = 'grider';
      else type = 'bomb';
      gems[type]++;
      animateGemDrop(type);
      playSound('gem');
  ...
  function gameOver() {
    ...
    setTimeout(() => {
      ...
      overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.9);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#00ffff;font-size:2rem;z-index:9999;';
      overlay.innerHTML = `
        <h2 style="font-size:4rem;margin:20px;">GAME OVER</h2>
        <p>Score: ${score}</p>
        <p>Highest Tile: ${highest || '—'}</p>
        <button onclick="location.reload()" style="margin-top:30px;padding:15px 40px;background:#00ffff;color:black;font-size:1.5rem;border:none;border-radius:50px;cursor:pointer;">RESTART THE GRID</button>
      `;
  ...
  // =============================================
  // SOUND SYSTEM — ALL EFFECTS INCLUDED
  // =============================================
  function playSound(type) {
    const sounds = {
      ...
      destroy: 'sounds/destroy.mp3',     // GRIDER
      bomb: 'sounds/bomb.mp3',           // BOMB
